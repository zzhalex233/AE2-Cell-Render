...
public final class CrazyAeDriveAdapter implements DriveAdapter {

    private static final String BLOCK_CLASS = "dev.beecube31.crazyae2.common.blocks.storage.BlockDriveImproved";
    private static final String TILE_CLASS = "dev.beecube31.crazyae2.common.tile.storage.TileImprovedDrive";
    private static final ResourceLocation MODEL_LOCATION = new ResourceLocation("crazyae", "builtin/driveimp");

    private final DriveRenderCache cache;
    private final SnapshotRequester snapshotRequester;

    public CrazyAeDriveAdapter(DriveRenderCache cache) {
        this(cache, new NetworkSnapshotRequester());
    }

    CrazyAeDriveAdapter(DriveRenderCache cache, SnapshotRequester snapshotRequester) {
        this.cache = cache;
    ...
    public int slotCount() {
        return DriveSlotLayouts.forId(DriveSlotLayouts.CRAZYAE_LAYOUT_ID).slotCount();
    }
    ...
    public DriveVisualState captureClientState(World world, BlockPos pos, TileEntity tile) {
        ...
        EnumFacing forward = ReflectiveDriveAccess.invokeFacing(tile, "getForward", EnumFacing.NORTH);
        EnumFacing up = ReflectiveDriveAccess.invokeFacing(tile, "getUp", EnumFacing.UP);
        return captureClientState(world, pos, tile, forward, up, world.provider.getDimension(), pos.toLong(), world.getTotalWorldTime(), slotCount);
    }

    DriveVisualState captureClientState(@Nullable World world, @Nullable BlockPos pos, Object drive, EnumFacing forward, EnumFacing up, int dimensionId, long positionKey, long worldTime) {
        int slotCount = ReflectiveDriveAccess.invokeInt(drive, "getCellCount", slotCount());
        return captureClientState(world, pos, drive, forward, up, dimensionId, positionKey, worldTime, slotCount);
    }

    private DriveVisualState captureClientState(@Nullable World world, @Nullable BlockPos pos, Object drive, EnumFacing forward, EnumFacing up, int dimensionId, long positionKey, long worldTime, int slotCount) {
    ...
    private List<DriveSlotVisual> buildSlotVisuals(@Nullable DriveRenderSnapshot snapshot, int slotCount) {
        ...
        for (int slot = 0; slot < slotCount; slot++) {
            ...
            slots.add(new DriveSlotVisual(slot, payload, DriveSlotLayouts.CRAZYAE_LAYOUT_ID));
        }
        return slots;
    }
